...
Markdown.dialects.Gruber = {
  ...
  inline: {
    __call__: function inline( text, patterns ) {
      ...
      while ( ( m = re.exec(text) ) != null) {
        ...
        if ( m[2] in this.dialect.inline) {
          res = this.dialect.inline[ m[2] ].call(this, text.substr( m.index + m[1].length ) ) ||
    ...
    "**": function strongStar( text ) {
    },

    "__": function strongUl( text ) {
    },

    "*": function emStar( text ) {
    },

    "_": function emUl( text ) {
    },
...
tests = {
  ...
  test_inline_br: tests.meta(function(md) {
    ...
    asserts.same( md.processInline("\\bar"), [ "\\bar" ] );
  })
}
