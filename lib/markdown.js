 ...
 *  to this module, it is first parsed into a markdown tree by calling
 *  [[parse]].
 **/
exports.toHTMLTree = function toHTMLTree( input, dialect ) {
...
Markdown.dialects.Maruku.block.block_meta = function block_meta( block, next ) {
  ...
  var attr = process_meta_hash( m[ 1 ] );
...
function split_args( argsString ) {
    // there are two halves to this regex.
    // matches quoted arguments:
    //    (['"])((?:\\\\|\\\1|[^\1])+)\1
    // matches unquoted arguments:
    //    (?:([^'"](?:\\\s|[^\s])+)(?:\s|$))
    var pattern = /\s*(?:(['"])((?:\\\\|\\\1|[^\1])+)\1|(?:([^'"](?:\\\s|[^\s])+)(?:\s|$)))\s*/g;
    
    var args = [];
    var result;
    while (result = pattern.exec(argsString)) {
        var arg = result[2] || result[3];
        args.push(arg.replace(/\\(['"\s\\])/g, "$1"));
    }
    return args;
};
